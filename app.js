...
function maybeAutoOpen(){return;},isMobile?12000:8000);}catch(e){}}
...
function bindConfirm(){var btn=document.getElementById('qbConfirm');if(!btn)return;btn.addEventListener('click',async function(){var phoneEl=document.getElementById('qbPhone');var nameEl=document.getElementById('qbName');var p=normPhone(phoneEl.value||'');if(!p){phoneEl.style.borderColor='#c00';phoneEl.focus();phoneEl.placeholder='Enter 10-digit phone';return;}QB.phone=p;QB.name=(nameEl.value||'').trim()||'Web booking';btn.disabled=true;btn.textContent='Booking...';try{await fetch('/api/book',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:QB.name,phone:QB.phone,when:QB.day+' at '+QB.time,size:'Not specified',pain:'Quick book (web)',source:'website-quickbook-v7',timestamp:new Date().toISOString()})});}catch(e){}document.getElementById('qbDayStep').style.display='none';document.getElementById('qbTimeStep').style.display='none';document.getElementById('qbContactStep').style.display='none';var done=document.getElementById('qbDoneStep');done.style.display='block';document.getElementById('qbDoneMsg').innerHTML='Tre will text and call to confirm <strong>'+QB.day+' at '+QB.time+'</strong>.<br>Confirmation text coming to '+QB.phone+'.<br><br>1809 Hwy 15 S, Sumter SC';});}
